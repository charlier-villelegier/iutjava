...
public class ApplicationLogs extends ArrayList<IApplicationLog> {
	...
	public ArrayList<IApplicationLog> getErrors() {
		...
		for (IApplicationLog listener_i : this) {
			if(listener_i instanceof ApplicationErrorLog){
				filteredLogs.add(listener_i);
			}
		}
		return filteredLogs;
	}
	public ArrayList<IApplicationLog> getWarnings() {
		...
		// une boucle à faire ici, penser à utiliser instanceof
		for (IApplicationLog listener_i : this) {
			if(listener_i instanceof ApplicationWarningLog){
				filteredLogs.add(listener_i);
			}
		}
		return filteredLogs;
	...
	public ArrayList<IApplicationLog> getInfos() {
		...
		// une boucle à faire ici, penser à utiliser instanceof
		for (IApplicationLog listener_i : this) {
			if(listener_i instanceof ApplicationInfoLog){
				filteredLogs.add(listener_i);
			}
		}
		return filteredLogs;
